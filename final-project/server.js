...
const AUTH_JWT_OPTIONS = { expiresIn: 180000 };
...
server.post(["/auth/login", "/auth/refresh-token"], function (req, res, next) {
  if (req.url === "/auth/login") {
    ...
    if (!req.user)
      return res.status(400).send("No user with those credentials!");
  }
